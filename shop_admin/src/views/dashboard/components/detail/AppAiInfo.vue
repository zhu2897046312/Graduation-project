...
import {getAppAiInfoApi} from "/@/api/core/databoard.ts";
...
const data = ref<any>([
  {
    tit: 'AI绘画',
    val: 89,
    key: 'app_text_to_image_count'
  },
  {
    tit: 'AI绘画用户',
    val: 89,
    key: 'app_text_to_image_user_count'
  },
  {
    tit: '真人转漫画',
    val: 89,
    key: 'app_image_to_image_count'
  },
  {
    tit: '真人转漫画用户',
    val: 89,
    key: 'app_image_to_image_user_count'
  }
])
const options = {
  tooltip: {
    ...
    options.series = [
      {
        name: 'AI绘画',
        type: 'line',
        data: data.map((it: any) => it.app_text_to_image_count),
        lineStyle: {
          color: 'red'
        },
        itemStyle: {
      ...
      },
      {
        name: '真人转漫画',
        type: 'line',
        data: data.map((it: any) => it.app_image_to_image_count),
        lineStyle: {
          color: 'blue'
        },
        itemStyle: {
...
const initChart = async (id: string) => {
  ...
  const res = await getAppAiInfoApi({

...
import {getMixInfoApi } from "/@/api/core/databoard.ts";
...
const data = ref<any>([
  ...
  },
  {
    tit: '混剪二创生成',
    val: 89,
    key: 'derivative_mix_count'
  },
  {
    tit: '生成用户',
    val: 89,
    key: 'derivative_mix_user_count'
  }
])
const options = {
  tooltip: {
  ...
  options.series = [
    {
      name: '混剪',
      type: 'line',
      data: data.map((it: any) => it.mix_count),
      lineStyle: {
        color: 'red'
      },
      itemStyle: {
    ...
    },
    {
      name: '混剪二创',
      type: 'line',
      data: data.map((it: any) => it.derivative_mix_count),
      lineStyle: {
        color: 'blue'
      },
      itemStyle: {

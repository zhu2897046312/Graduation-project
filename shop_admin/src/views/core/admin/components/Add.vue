<template>
  ...
  >
    <Form ref="formRef" v-bind="formProp" :model="formState" :rules="rules">
      <Form.Item name="admin_status" button-style="solid" label="状态">
        <Radio.Group v-model:value="formState.admin_status">
      ...
      <Form.Item name="pwd" label="登陆密码">
        <Input.Password v-model:value="formState.pwd" />
      </Form.Item>
      ...
      <Form.Item name="roles" label="分配角色">
        <Select v-model:value="formState.roles" mode="tags" multiple>
          <Select.Option v-for="(item, index) in roles" :key="index" :value="item.id">
            {{ item.role_name }}
          </Select.Option>
        </Select>
      </Form.Item>
...
<script lang="ts" setup>
  ...
  const handleOpen = () => {
    loadingTask(async () => {
      ...
      // liaison_station_list.value = await apiFormatApiLine();
  ...
  const handleSubmit = async () => {
    ...
    loadingTask(async () => {
      await coreAdminCurdApi('CREATE', values);

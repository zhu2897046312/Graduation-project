<template>
  <BasicModal title="标签信息" @on-ok="handleSubmit" @on-close="handleClose" ref="modalRef">
...
const handleOpen = (e: any) => {
  loadingTask(async () => {
    formState.value = await api.cms.tag.curdApi('INFO', e.id)
    id = e.id;
...
const handleSubmit = async () => {
  ...
  values.id = id
  loadingTask(async () => {
    await api.cms.tag.curdApi('MODIFY', values);

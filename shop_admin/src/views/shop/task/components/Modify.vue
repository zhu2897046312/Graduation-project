...
const handleOpen = (e: any) => {
  loadingTask(async () => {
    ...
    formState.value = await api.shop.task.info(e.id)
    formState.value.category_id = JSON.parse(formState.value.config).categoryId
    formState.value.q = JSON.parse(formState.value.config).q
    formState.value.num = JSON.parse(formState.value.config).num
    formState.value.ids = formState.value.type === 2 ? JSON.parse(formState.value.config).ids.join('\n') : ''
    console.debug(formState.value)
    id = e.id;
...
const handleSubmit = async () => {
  ...
  values.id = id
  loadingTask(async () => {
    await api.shop.task.modify(values);

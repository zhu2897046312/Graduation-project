...
const handleSubmit = async () => {
  try {
    ...
    post_data.id = id;
    await api.shop.product.modify(post_data)
...
const handleLoadData = async() => {
  const res: any = await api.shop.product.info(id)
  data.value = {
    category_id: res.product.category_id,
    title: res.product.title,
    state: res.product.state,
    ...
    picture_gallery: res.product.picture_gallery,
    description: res.product.description,
    sold_num: res.product.sold_num,
    sort_num: res.product.sort_num,
    putaway_time: dayjs(res.product.putaway_time),
    content: res.content.content,
    seo_title: res.content.seo_title,
    seo_keyword: res.content.seo_keyword,
    seo_description: res.content.seo_description,
    property_list: res.property_list,
    open_sku: res.product.open_sku,
    sku_list: res.sku_list,
    tags: res.tags ? res.tags.map(it => it.id) : [],
    hot: res.product.hot,
  }
  if (null != res.sku_config_list) {
    initSkuConfig(res.sku_config_list);
  }
}

const initSkuConfig = (sku_config_list : any[]) => {
  const _config = new Map<number, number[]>()
  for (const item of sku_config_list) {
    let values: number[] = []
    if (_config.has(item.prod_attributes_id)) {
      values = _config.get(item.prod_attributes_id)!
    }
    values.push(item.prod_attributes_value_id)
    _config.set(item.prod_attributes_id, values)
  }
  sku_config.value = _config
}


onMounted(async () => {
  id = parseInt(route.query.id ? route.query.id.toString() : '0')
          ...
          <div class="full-width flex gap-4">
            <div class="w-9/12">
              ...
              <Form.Item label="商品内容" name="content" :label-col="layout.mainLabel" >
                <Editor v-if="data" v-model:value="data.content" />
              </Form.Item>

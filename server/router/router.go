...
func SetupRouter(factory *service.ServiceFactory) *gin.Engine {
	r := gin.Default()

	// 地点路由组
	placeHandler := handlers.NewCmsPlaceHandler(factory.GetCmsAssociatedPlaceService())
	placeGroup := r.Group("/cms/places")
	{
	...
	categoryGroup := r.Group("/cms/categories")
	{
	...
	commentGroup := r.Group("/cms/comments")
	{
	...
	archiveGroup := r.Group("/cms/archives")
	{
	...
	documentGroup := r.Group("/cms/documents")
	{
	...
	docTagGroup := r.Group("/cms/document-tags")
	{
	...
	recIndexGroup := r.Group("/cms/recommend-indices")
	{
	...
	recommendGroup := r.Group("/cms/recommends")
	{
	...
	spotGroup := r.Group("/cms/scenic-spots")
	{
	...
	likeHistoryGroup := r.Group("/cms/user-like-histories")
	{
	...
	// 管理员路由组
	adminHandler := handlers.NewCoreAdminHandler(factory.GetCoreAdminService())
	adminGroup := r.Group("/core/admins")
	{
	...
	adminRoleGroup := r.Group("/core/admin-roles")
	{
	...
	configGroup := r.Group("/core/configs")
	{
	...
	permissionGroup := r.Group("/core/permissions")
	{
	...
	payConfigGroup := r.Group("/mp/pay-configs")
	{
	...
	productGroup := r.Group("/mp/products")
	{
	...
	resetTokenGroup := r.Group("/mp/reset-tokens")
	{
	...
	userTokenGroup := r.Group("/mp/user-tokens")
	{
	...
	paypalOrderLogsGroup := r.Group("/paypal/order-logs")
	{
	...
	paypalWebhookLogsGroup := r.Group("/paypal/webhook-logs")
	{
	...
	tagIndexGroup := r.Group("/shop/tag-indices")
	{
	...
	tagMateGroup := r.Group("/shop/tag-mates")
	{
	...
	spCategoryGroup := r.Group("/sp/categories")
	{
	...
	spOrderItemGroup := r.Group("/sp/order-items")
	{
	...
	spOrderHistoryGroup := r.Group("/sp/order-histories")
	{
	...
	spAddressGroup := r.Group("/sp/addresses")
	{
	...
	spRefundGroup := r.Group("/sp/refunds")
	{
	...
	spOrderGroup := r.Group("/sp/orders")
	{
	...
	spAttrGroup := r.Group("/sp/attributes")
	{
	...
	spAttrValueGroup := r.Group("/sp/attribute-values")
	{
	...
	spContentGroup := r.Group("/sp/product-contents")
	{
	...
	productPropertyGroup := r.Group("/sp/product-properties")
	{
	...
	// productGroup := r.Group("/sp/products")
	// {
	// 	productGroup.POST("", productHandler.CreateProduct)
	// 	productGroup.PUT("/:id", productHandler.UpdateProduct)
	// 	productGroup.GET("/:id", productHandler.GetProduct)
	...
	addressGroup := r.Group("/sp/user-addresses")
	{
	...
	cartGroup := r.Group("/sp/user-carts")
	{

...
func (s *SpCategoryService) GetCategoryByID(id common.MyID) (*sp.SpCategory, error) {
	if id == 0 {
...
func (s *SpCategoryService) GetCategoriesByPid(pid common.MyID) ([]*sp.SpCategory, error) {
	return s.repoFactory.GetSpCategoryRepository().FindByPid(pid)
}

// UpdateCategoryState 更新分类状态
func (s *SpCategoryService) UpdateCategoryState(id common.MyID, state uint8) error {
	if state != 0 && state != 1 {
		return errors.New("无效的状态值")
	}
...
func (s *SpCategoryService) UpdateCategorySortNum(id common.MyID, sortNum uint16) error {
	if sortNum > 9999 {

...
import api from '../../api'
import type { Category, ProductListResponse, ProductItem } from '../../types/type'
...
// 获取商品列表
const { data: productData, pending: productsPending } = await useAsyncData<ProductListResponse>(
  `products-${code}`,
  async () => {
    if (!info.value) {
      return {
...
<template>
  <div>
    ...
    <!-- 加载状态 -->
    <div
      v-if="categoryPending || productsPending"

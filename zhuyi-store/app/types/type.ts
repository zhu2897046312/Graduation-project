...
/** useCartList 的选项（与 Nuxt useAsyncData 的 watch 等选项对齐） */
export interface UseCartListOptions {
  lazy?: boolean
  /** 监听源数组，类型与 useAsyncData 的 watch 一致 */
  watch?: (import('vue').WatchSource<unknown> | object)[]
...
export interface CategoryTreeNode {
  node: Pick<Category, 'code' | 'title'> & Record<string, unknown>
...
/** SKU 规格前端 VO 的 value 项（含选中/禁用状态） */
export interface SpProductProdFrontVoValueItem extends SkuConfigValue {
  selected?: boolean
  disabled?: boolean
}

/** SKU 规格前端 VO（商品详情规格选择器） */
export interface SpProductProdFrontVo {
  id: number
  title: string
  value: SpProductProdFrontVoValueItem[]
}
...
/** Footer 文档项（CmsDocument + link） */
export type FooterDocumentItem = CmsDocument & { link: string }

/** 表单校验规则：接收字符串，返回 true 或错误文案 */
export type ValidationRule = (v: string) => true | string

/** 结账页地址表单状态（与 errors 的 key 一致，便于按字段校验） */
...
/** Navigator 扩展（Device Memory API，部分浏览器支持） */
